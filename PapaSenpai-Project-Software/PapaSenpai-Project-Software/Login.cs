...
namespace PapaSenpai_Project_Software
{
    public partial class Login : MaterialSkin.Controls.MaterialForm
    {
        ...
        private void userLogin()
        {
            ...
            if (user.HasRows)
            {
                Admin admin = new Admin(Convert.ToInt32(user["id"]),user["username"].ToString(), user["role_title"].ToString(), user["first_name"].ToString(), user["last_name"].ToString(), user["email"].ToString());
