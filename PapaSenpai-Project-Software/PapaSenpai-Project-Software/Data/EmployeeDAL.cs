...
namespace PapaSenpai_Project_Software.Data
{
    public class EmployeeDAL : BaseDAL
    {
        ...
        {
            return executeReader("SELECT employees.*, departments.title as department_title, contracts.title as contract, COUNT(schedules_employees.id) as shifts_taken," +
                " SUM(schedules_employees.hours_worked) as hours_worked" +
                " FROM employees " +
