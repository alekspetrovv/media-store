...
namespace PapaSenpai_Project_Software.Data
{
    public class EmployeeDAL : BaseDAL
    {
        ...
        {
            executeNonQuery("INSERT INTO `employees`(`first_name`, `last_name`, `address`, `city`, `country`," +
        ...
        public void Update(string[] employee_bindings)
        {
            ...
            executeNonQuery("UPDATE `employees` SET `first_name`= @firstname,`last_name`= @secondname," +
        ...
        {
                   return executeReader("SELECT employees.*, departments.title as department , contracts.title as contract, COUNT(schedules_employees.id) as shifts_taken, SUM(schedules_employees.hours_worked) as hours_worked" +
                       " FROM employees " +
